...
namespace ConvertFoldersToWebP {
    public partial class MainWindow : Window {
        ...
        private string SetOutputFilePath(string file) {
            if (_OutputFolder != string.Empty && chkSaveFolder.IsChecked == true ) {
        ...
        private void OpenErrorLog_Click(object sender, RoutedEventArgs e) {
            // Retrieve the file path from the TextBox
            string filePath = txtErrorLog.Text;
            ...
            if (!string.IsNullOrWhiteSpace(filePath)) {
                try {
                    ...
                    // Handle exceptions like file not found, permission issues, etc.
                    System.Windows.MessageBox.Show($"An error occurred while trying to open the file: {ex.Message}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        }
    }
